...
const urls = [
  "https://blog.naver.com/bright__xo/224397501294",
  "https://blog.naver.com/staow1517/224188635485",
  "https://blog.naver.com/hochul1005/224101453925",
];

const previews = await Promise.all(urls.map((url) => fetchBlogPreview({ url })));
for (const preview of previews) {
  const text = `${preview.title ?? ""} ${preview.description ?? ""}`;
  if (!text.includes("푸른마당") || !/의정부|민락/.test(text)) {
...
const { data: court, error: courtError } = await supabase
  ...
  .eq("basic_city", "의정부시")
  .eq("basic_court_name", "푸른마당테니스장")
  .single();
if (courtError) throw new Error(`푸른마당테니스장 조회 실패: ${courtError.message}`);
